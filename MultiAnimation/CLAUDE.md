...
| `test_camera_capture.lua` | Camera-capture drift regression: navigation leaves unmoved camera keyframes bit-stable (with and without Look Through), cut mode + easing survive, Focus-in-front contract, Look Through fly rewrites only its frame, empty-frame pass-through creates nothing, fly at empty frame does. Appends its frames at the timeline end so it runs regardless of session content (16 cases, live) |
...
Suite total: **~1006 cases** across 42 runnable files (2 skipped headless: `test_player` → `mcp playtest`, `test_scrubber` → interactive; noticeably fewer when the live session holds user data — several `test_ui_simple` blocks skip). Note: `test_ui_playback` test 19 now asserts fps is absent from snippet; `test_ui_simple` insert-frame tests now assert Duplicate (cursor to frame+1, frame+1 has data). `run_tests.py` pre-flights a local copy-sync check (easedAlpha/cubicCF/smoothCF/smoothV3 across the three game players AND their test inline copies, LEGACY_POSE_TO_JOINT across player+data-server, spawned-effect PRESETS plugin↔test, lerpState/applyPartState across Interpolator/PropCapture + both game players + test_prop_state; comment/whitespace/trailing-comma-insensitive) — deliberate duplicates that used to drift silently now fail the run. Test files must end with the standard `=== N passed, M failed ===` summary line — `run_tests.py` parses it for the counts (a file missing it reports 0/0).
