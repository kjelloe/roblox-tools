...
```bash
python3 run_tests.py [pattern] [-v]   # full suite (353 cases), or `mcp test`
...
| `test_ui_simple.lua` | Simple Mode bridge integration: mode toggle, idempotent step-forward capture, Delete Keyframe redo, Play/Stop toggle, manipulable camera object (FOV, Look Through, capture-from-gizmo), FIGURES auto-track/untrack (47 cases) |

Suite total: **353 cases** across 19 files (2 skipped headless: `test_player` → `mcp playtest`, `test_scrubber` → interactive).
