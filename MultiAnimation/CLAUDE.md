...
```bash
python3 run_tests.py [pattern] [-v]   # full suite (358 cases), or `mcp test`
...
| `test_ui_simple.lua` | Simple Mode bridge integration: mode toggle, idempotent step-forward capture, Delete Keyframe redo, Play/Stop toggle, manipulable camera object (FOV, frustum gizmo, Look Through guard/snap/free-fly-mirrors-to-gizmo/restore, capture-from-gizmo), FIGURES auto-track/untrack (53 cases) |

Suite total: **358 cases** across 19 files (2 skipped headless: `test_player` → `mcp playtest`, `test_scrubber` → interactive).
