...
> **Full rotations (flips, spins):** in-game playback uses **smooth
> (Catmull-Rom-style) interpolation by default**, so sparse keyframes play as
> fluid arcs — but keep rotation steps under ~120° per keyframe, and use
> `+ Insert` to open a gap mid-move when you need more (events and later
> frames shift along automatically). Editor Preview is still linear, so judge
> final smoothness in Play mode. Pass `smooth = false` in the play opts for
> legacy linear playback.
